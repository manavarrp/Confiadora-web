...
import { getPersonFormPayload } from '../../../../utils/physicalPersonToTransformData'
import MaritalStatus from '../../../Catalogs/MaritalStatus'
...
function RegisterForms ({ initialValues }) {
  ...
  const onSubmitPersonForm = async (data) => {
    const { error, files, payload } = await getPersonFormPayload({ customerId, ...data })
    if (error) return toast.warning('Se ha producido un error al cargar tu archivo intenta nuevamente por favor')
    console.log({ data })
    postAttechmetsCustomer(payload, files, '/natural-person/personal-data')
  }
    ...
    <div className=''>
      <FormProvider {...methods}>
        <form className='mx-auto max-w-screen-md'>
          ...
          <div className='flex flex-col w-full mb-3 gap-3 sm:flex-row'>
            <div className='flex flex-col w-1/2'>
              <label className='text-sm text-darkBlue'>Estado Civil</label>
              <MaritalStatus />
            </div>
            <div className='flex flex-col w-1/2'>
              <label className='text-sm text-darkBlue'>FIEL</label>
              <Input
                type='text'
                placeholder='fiel'
                className={styles.textbox}
