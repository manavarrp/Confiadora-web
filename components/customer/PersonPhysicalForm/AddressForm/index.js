...
import { getAddressPayload } from '../../../../utils/physicalPersonToTransformData'
...
function RegisterForms ({ initialValues }) {
  ...
  const onSubmitAddressFormUpdate = async (data) => {
    const { error, files, payload } = await getAddressPayload({ customerId, ...data })
    if (error) return toast.warning('Se ha producido un error al cargar tu archivo intenta nuevamente por favor')

    postAttechmetsCustomer(payload, files, 'natural-person/address-information')
  }
  ...
  return (
    <div className=''>
      <FormProvider {...methods}>
        ...
        <ModalResetData
          onClose={() => {
            setModal({ open: false, callBack: null })
          }}
